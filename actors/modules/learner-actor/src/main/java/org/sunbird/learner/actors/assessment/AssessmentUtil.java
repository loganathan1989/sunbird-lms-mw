...
public class AssessmentUtil{
	 ...
	 * evaluation is done data will be stored into different-2 
	 * tables with there evaluated result.
	...
	/**
	 * This method will fetch user all unevaluated assessment data. 
	 * request map will contain courseId, contentId, userId values only.
	 * @param request Map<String, Object>
	...
	/**
	 * This method will update user assessment score under content consumption table.
	 * @param assmntListResult List<Map<String, Object>>
	 */
	@SuppressWarnings("unchecked")
  private void updateScoreIntoContentConsumptionTable(List<Map<String, Object>> assmntListResult) {
    ...
    for (Map<String, Object> map : assmntListResult) {
      ...
      String id = OneWayHashing.encryptVal(userId + JsonKey.PRIMARY_KEY_DELIMETER + contentId
          + JsonKey.PRIMARY_KEY_DELIMETER + courseId);
      ...
      reqMap.putAll(map);
      try {
        Response response = cassandraOperation.getRecordById(contentConsumptionDbInfo.getKeySpace(),
            contentConsumptionDbInfo.getTableName(), id);
        List<Map<String, Object>> responseList =
            (List<Map<String, Object>>) response.get(JsonKey.RESPONSE);
        if (null != responseList && responseList.isEmpty()) {
          cassandraOperation.upsertRecord(contentConsumptionDbInfo.getKeySpace(),
              contentConsumptionDbInfo.getTableName(), reqMap);
        } else if (null != responseList && responseList.size() == 1) {
          Map<String, Object> contentConsumptionMap = responseList.get(0);
          if (((Double.parseDouble((String) contentConsumptionMap.get(JsonKey.ASSESSMENT_SCORE)))
              - (Double.parseDouble((String) reqMap.get(JsonKey.ASSESSMENT_SCORE)))) < 0) {
            cassandraOperation.upsertRecord(contentConsumptionDbInfo.getKeySpace(),
                contentConsumptionDbInfo.getTableName(), reqMap);
          }
        }
      ...
      try {
        ...
        if (null != resList && resList.size() > 0) {
          ...
          Double scoreFrmDbRes = (resMap.get(JsonKey.ASSESSMENT_SCORE)) != null
              ? Double.parseDouble((String) resMap.get(JsonKey.ASSESSMENT_SCORE)) : 0d;
          Double scoreFrmReq = (map.get(JsonKey.ASSESSMENT_SCORE)) != null
              ? Double.parseDouble((String) map.get(JsonKey.ASSESSMENT_SCORE)) : 0d;

          if (scoreFrmDbRes > scoreFrmReq) {
            Map<String, Object> reqMap = new HashMap<>();
            int attemptedCount = (resMap.get(JsonKey.ATTEMPTED_COUNT) != null)
                ? (int) resMap.get(JsonKey.ATTEMPTED_COUNT) : 0;
            reqMap.put(JsonKey.ATTEMPTED_COUNT, attemptedCount + 1);
            reqMap.put(JsonKey.ID, resMap.get(JsonKey.ID));
            cassandraOperation.updateRecord(assmntEvalDbInfo.getKeySpace(),
                assmntEvalDbInfo.getTableName(), map);
          } else {
          ...
          cassandraOperation.insertRecord(assmntEvalDbInfo.getKeySpace(),
              assmntEvalDbInfo.getTableName(), map);
        }
      } catch (Exception e) {
        ProjectLogger.log(e.getMessage(), e);
