package org.sunbird.user.actors.bulkupload;
...
import org.sunbird.user.util.UserUtility;
...
public class UserDataEncryptionDecryptionServiceActor extends UntypedAbstractActor {
  ...
  private void decryptUserDataAndUpdateDb(Map<String, Object> userMap) {
    try {
      ...
      ProjectLogger.log("Exception Occurred while decrypting user data for userId "
          + ((String) userMap.get(JsonKey.ID)), e);
    }
  ...
  private void getUserAddressDataDecryptAndUpdateDb(String userId) {
    ...
    try {
      ...
      for (Map<String, Object> address : addressList) {
        cassandraOperation.updateRecord(addrDbInfo.getKeySpace(), addrDbInfo.getTableName(),
            address);
      }
    } catch (Exception e) {
      ProjectLogger
          .log("Exception Occurred while decrypting user address data for userId " + (userId), e);
    }
