...
public class RequestRouter extends UntypedAbstractActor {
  ...
  static {
      try (InputStream inputStream = RequestRouter.class.getClassLoader().getResourceAsStream("actor-config.xml")) {
          ...
          for (int temp = 0; temp < nList.getLength(); temp++) {
              ...
              if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  try {
                      ...
                      if (StringUtils.equalsIgnoreCase("RequestRouter", name) || StringUtils.equalsIgnoreCase("LearnerRouter", name)
                              || StringUtils.equalsIgnoreCase("UserRouter", name)) {
                          Class<?> cls = Class.forName(className);
                          Props actorProps = Props.create(cls);
                          ActorRef actor = getContext().actorOf(new SmallestMailboxPool(count).props(actorProps));
                              ...
                              if (null != graphList && graphList.getLength() > 0) {
                                  for (int j = 0; j < graphList.getLength(); j++) {
                                      ...
                                      if (gNode.getNodeType() == Node.ELEMENT_NODE) {
                                          ...
                                          addActorsToPool(arr, count, id, addresses);
                                      }
                                  }
                              } else {
                                  addActorsToPool(arr, 1, null, null);
                              }
                          }
                      }
                  }
