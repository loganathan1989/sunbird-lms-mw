...
public class RecommendorActor extends UntypedAbstractActor {
  ...
  private void getRecommendedCourses(Request request) {
    ...
    if (null == map) {
      ...
      ProjectCommonException exception =
          new ProjectCommonException(ResponseCode.invalidUserCredentials.getErrorCode(),
    ...
    try {
      ...
      response.put(JsonKey.RESPONSE, shuffleList(
    ...
    if (response.getResult() != null) {
      if (response.getResult().get(JsonKey.RESPONSE) != null) {
        ...
        if (userDBO != null && userDBO.size() > 0) {
          // TODO: get the subject,grade , language from userDBO(database object) and create the
          // response ...
